...
test('Preload canvas', async () => {
  const page = await electronApp.firstWindow();
  const canvasElement = page.locator('div#Canvas');
  expect(await canvasElement.count(), 'expect find one element #Canvas').toStrictEqual(1);
});

// test('Preload versions', async () => {
//   const page = await electronApp.firstWindow();
//   const versionsElement = page.locator('#versions');
//   expect(await versionsElement.count(), 'expect find one element #versions').toStrictEqual(1);
