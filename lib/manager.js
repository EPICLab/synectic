...
class AppManager {
  ...
  //context menu for a lasso-drawn rectangle around the cards
  rectangleMenuBuilder() {
    let RectangleContextMenu = new Menu()
    RectangleContextMenu.append(new MenuItem(
      {'label': 'Create Stack', 
        click() {
          let cards = $('.ui-selected')
          if ( cards.length < 2 ) {
             alert("Cannot create Stack with just two cards")
             return -1
          }
          let s = new Stack()({
            card1: cards[0],
    ...
    RectangleContextMenu.append(new MenuItem(
      {'label': 'Create Group',
        click() { console.log("Create Group bhai!") }
      })
    )

    return RectangleContextMenu;
  }
