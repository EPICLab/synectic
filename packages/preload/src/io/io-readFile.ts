import {readFile, type PathLike} from 'fs-extra';
...
export function readFileAsync(filepath: PathLike): Promise<Buffer>;
/**
 * Asynchronously read file contents into a Buffer or string.
 * @param filepath A valid filename or path to read from.
 * @param options Options object for setting either file encoding (default: `null`) or file system
 * flags (default: `"r"`). The possible option values are:
 *
 ...
 * | `"wx+"` | Like `"w+"` but fails if the path exists.                                                                                  |
 * @returns {Promise<string>} A Promise object containing a Buffer, if no flag or encoding was
 * provided, or a string of the file contents. Throws ENOENT error on non-existent filepath, and
 * EISDIR error on a filepath pointing to a directory.
 */
export function readFileAsync(
  filepath: PathLike,
  options?: {flag: string} | {encoding: NodeEncoding; flag?: string},
  ...
  if (options) return readFile(fullPath, options);
  else return readFile(fullPath);
}
