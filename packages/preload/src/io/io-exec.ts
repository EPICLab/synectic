...
const promiseExec = promisify(exec); // requires nodeIntegration: true
...
/**
 * Spawns a shell then executes the command within that shell, buffering any generated output. The
 * command string passed to the exec function is processed directly by the shell and special
 * characters (vary based on shell) need to be dealt with accordingly.
 *
 * WARNING: Never pass unsanitized user input to this function. Any input containing shell
 * meta-characters may be used to trigger arbitrary command execution.
 *
 ...
 * @param obj.debug - A debug flag for printing the formatted command before execution; defaults to
 *   `false`.
 * @returns {Promise<{ stdout: string | undefined; stderr: string | undefined }>} A Promise object
 *   containing the `stdout` and `stderr` strings with content based on the output of the command.
 */
 ...
 * otherwise.
 */
const isWrappedQuote = (maybeQuote: string): boolean => {
  return (
