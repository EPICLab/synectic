import {pathExists, type PathLike} from 'fs-extra';
...
export const getMatchUp = async (
  ...
  return await lookItUp(async dir => {
    return (await matcher(dir)) ? dir : null;
  }, filepath.toString());
};
...
export const getRoot = async (filepath: PathLike): Promise<PathLike | undefined> => {
  ...
  const dir = await lookItUp(async directory => {
    const targetPath = join(directory, '.git');
    return (await pathExists(targetPath)) ? directory : null;
  }, filepath.toString());
