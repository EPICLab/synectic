import type {PathLike, UUID} from '@syn-types/app';
import type {Commit} from '@syn-types/commit';
import type {ExactlyOne, Prettify} from '@syn-types/util';
...
import {commitAdded, commitRemoved} from '../slices/commits';
import {isVersionedMetafile} from '../slices/metafiles';
import {type BranchIdentifiers, updateBranches} from './branches';
import {updateFilebasedMetafile, updateVersionedMetafile} from './metafiles';
import {fetchRepo} from './repos';
import {catFile, commit, getConfig, removeUndefined, reset, revList} from '#preload';
...
export const buildCommit = createAppAsyncThunk<Commit, CommitIdentifiers>(
  ...
  async ({oid, root}, thunkAPI) => {
    const objectInfo = await catFile({dir: root, objectRef: oid});
...
>('commits/addCommit', async ({repoId, branchId, message}, thunkAPI) => {
  ...
  const oid = await commit({dir: branch.root, message: message});
  if (!oid) {
    console.error(`Unable to commit to ${branch.root}...`);
    return undefined;
  }
  const commitObj = await thunkAPI
...
export const revertCommit = createAppAsyncThunk<boolean, CommitIdentifiers>(
  ...
  async ({oid, root}, thunkAPI) => {
    ...
    const succeeded = await reset({dir: root, mode: 'soft', commit: oid});
    if (succeeded) {
