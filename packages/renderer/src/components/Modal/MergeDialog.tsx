import {isDefined, revList} from '#preload';
import {
  Box,
  Button,
  Dialog,
...
import type {MergeOutput, UUID} from '@syn-types/app';
import type {Branch} from '@syn-types/branch';
import type {MergeDialog as MergeDialogModal} from '@syn-types/modal';
...
import {GitAbort, GitBranch, GitMerge} from '../GitIcons';
import MergeStatus from './MergeStatus';

const MergeDialog = ({modal}: {modal: MergeDialogModal}) => {
  const title =
    modal.mode === 'abort' ? 'Abort Merge' : modal.mode === 'continue' ? 'Continue Merge' : 'Merge';
  const repo = useAppSelector(state => repoSelectors.selectById(state, modal.repo));
  const base = useAppSelector(state => branchSelectors.selectById(state, modal.base));
  ...
  const toggle = async (row: UUID) => {
    if (isDefined(modal.mode)) return;
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{px: 2, pb: 1.5, overflowY: 'hidden'}}>
        ...
        >
          {isDefined(modal.mode) ? 'Relevant' : 'Select'} branches from the{' '}
          <Box
            component="span"
        ...
        <TableContainer component={Paper}>
          <Table
            size="small"
            aria-label="merge-dialog-options"
          >
            <TableHead>
              <TableRow>
      ...
      <DialogActions sx={{px: 2, pb: 1.5}}>
        ...
        {modal.mode === 'abort' && !isDefined(result) ? (
          <Button
            id="abort-button"
            variant="contained"
            onClick={handleAbort}
        ...
        {!isDefined(modal.mode) && !isDefined(result) ? (
          <Button
            id="merge-button"
            variant="contained"
