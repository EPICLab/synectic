 ...
 * @param obj.ref - The git ref or symbolic-ref (i.e. `HEAD` or `HEAD~1`) to resolve against the current branch.
 * @param obj.depth - How many symbolic references to follow before returning.
 * @returns {Promise<string>} A Promise object containing the SHA-1 hash or branch name associated with the given `ref` depending on the `depth` parameter; 
 ...
 * @returns {Promise<isogit.TreeEntry | undefined>} A Promise object containing the SHA-1 hash associated with the filepath in the latest commit on the indicated branch.
 */
 ...
 * @returns {Promise<isogit.ReadCommitResult[]>} A Promise object containing a list of commits not found in both branches (i.e. the divergent set).
 */
 ...
 * @returns {Promise<BranchDiffResult[]>} A Promise object containing a list of file paths and a status indicator, where the possible status values for each file are:
 *
 * | status                | description                                                                           |
 * | --------------------- | ------------------------------------------------------------------------------------- |
 ...
 * 
 * @deprecated Use {@link gitIgnore.getIgnore} instead.
 ...
 * by three integers representing the HEAD status, WORKDIR status, and STAGE status of the entry (in this order). For each status, the 
 * values represent:
 ...
 * directory under version control, or a git status indicator (see `GitStatus` type definition for all possible status values).
 */
