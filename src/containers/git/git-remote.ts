...
import { execute, isDefined, removeUndefined, removeNullableProperties } from '../utils';
...
export const processRemoteOutput = (output: string): RemoteOutput[] => {
    const result: RemoteOutput[] = output.split(/\r?\n/).map(line => {
        /**
        ...
        return name ? { remote: name, ...removeNullableProperties({ url: url, type: isRemoteType(type) ? type : undefined }) } : undefined;
    }).filter(isDefined);
    return result;
};