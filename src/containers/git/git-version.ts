...
export const checkGitVersion = async (range: string, verbose = false): Promise<boolean> => {
    ...
    if (!satisfied) console.error(`git version ${version.version} does not meet minimum range ${range}`);
    if (verbose && satisfied) console.log(`git version ${version.version} satisfies minimum range ${range}`);
    return satisfied;
};