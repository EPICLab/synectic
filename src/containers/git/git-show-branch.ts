...
import { Expand, WithRequired, execute, isDefined, removeUndefined } from '../utils';
...
export const showBranch = async ({
    dir, all = false, remotes = false, list = true
}: {
    dir: PathLike;
    all?: boolean;
    remotes?: boolean;
...
export const processShowBranchOutput = (output: string, remotes: string[]): BranchOutput[] => {
    const result: BranchOutput[] = output.split(/\r?\n/).map(line => {
         ...
         */
        const linePattern = new RegExp(`^(?:(\\*)|(?: )) \\[(?:(${remotes.join('|')})/)?(.*)\\]`, 'g');
