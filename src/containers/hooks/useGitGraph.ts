...
const useGitGraph = (repoId: UUID, pruned = false): useGitGraphHook => {
  ...
  const staged = useAppSelector(state =>
    metafileSelectors.selectStagedByRepo(state, repo?.id ?? '')
  );
  ...
  useEffect(() => {
    const changed = prevStaged
      ? !filteredArrayEquality(prevStaged, staged, ['id', 'repo', 'branch', 'status'])
      : true;
  ...
  const process = async (target: 'branches' | 'staged') => {
    if (repo) {
      ...
      const { added, removed, modified } =
        target === 'branches' ? partition() : { added: [], removed: [], modified: [] };

      if (removed) {
        // no guarantee about providence of commits, invalidate graph and reconstruct
        newGraph.clear();
  ...
  const parseStaged = async (graph: GitGraph, branch: Branch) => {
    if (branch.scope === 'local') {
      ...
      const hasStaged = statuses
        ? statuses.entries.filter(e => staged.includes(e.status)).length > 0
        : false;
  ...
  const prune = (graph: GitGraph) => {
    const prunable = Array.from(graph.values()).filter(
      vertex => vertex.parents.length == 1 && vertex.children.length == 1
    );
  ...
  const partition = (): { added: Branch[]; removed: boolean; modified: Branch[] } => {
    const removed = prevBranches
      ? prevBranches.some(branch => !branches.some(b => b.id === branch.id))
      : false;
    ...
    const isModified = (prev: Branch, curr: Branch) => {
      ...
      const sameCommits = !diffArrays(prev.commits, curr.commits, {
        comparator: (a, b) => a.oid === b.oid
      }).some(d => d.added || d.removed);
    ...
    const [added, modified] = branches.reduce(
      (accumulator: [Branch[], Branch[]], branch) => {
        ...
        return !prev
          ? (accumulator[0].push(branch), accumulator)
          : modified
          ? (accumulator[1].push(branch), accumulator)
          : accumulator;
      },
      [[], []]
    );
