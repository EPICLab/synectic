...
import { MockedRepository, mockGit, MockInstanceEnhanced } from './mock-git';
...
export type DirectoryItem = string | Buffer | File | Directory | SymbolicLink | DirectoryItems;
...
export async function mock(
  config: DirectoryItems,
  gitRepo?: MockedRepository | undefined
): Promise<MockInstance | MockInstanceEnhanced> {
...
export const create = async ([filename, config]: [string, DirectoryItem]): Promise<void> => {
  if (isContent(config)) {
    ...
    if (items.length > 0) {
      for (const [subfile, subconfig] of items)
        await create([path.join(filename, subfile), subconfig]);
    } else {
