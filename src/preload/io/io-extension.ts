...
/**
 * Extract the file extension from the path. Returns the extension after the last period character
 * in the path, otherwise returns full path if first character is a period or no period exists.
 * Exhibits different behavior from `path.extname` in order to allow config files (e.g. '.htaccess'
 * returns 'htaccess', instead of '').
 *
