...
const deepEntitiesEqualityCheck = (a: BranchEntities, b: BranchEntities) =>
    filteredArrayEquality(Object.values(a).filter(isDefined), Object.values(b).filter(isDefined), ['id', 'head', 'status']);
const deepBranchesEqualityCheck = (a: Branch[], b: Branch[]) => filteredArrayEquality(a, b, ['id', 'head', 'status']);
...
const selectByRef = createCachedSelector(
    ...
    (branches, root, ref, scope) => branches.find(b => isEqualPaths(b.root, root) && b.ref === ref && b.scope === scope)
)({
    keySelector: (_, root, ref: string, scope: Branch['scope']) => `${root.toString()}:${ref}:${scope}`,
    selectorCreator: createSelectorCreator(defaultMemoize, { equalityCheck: deepBranchesEqualityCheck })
});
...
const selectByRepo = createCachedSelector(
    ...
    (branches, repo, dedup) => branches.reduce((accumulator: Branch[], branch) => {
        if (repo?.local.includes(branch.id)) {
            ...
            return dedup ? updated.filter(b => !(b.scope === 'remote' && b.ref === branch.ref)) : updated;
        }
        if (repo?.remote.includes(branch.id)) {
            // if dedup, then prefer local branch and only add remote branch if no existing match
            return (dedup && !accumulator?.some(b => b.scope === 'local' && b.ref === branch.ref)) || !dedup ?
                (accumulator.push(branch), accumulator) : accumulator;
        }
        return accumulator;
    }, [])
)({
    ...
    selectorCreator: createSelectorCreator(defaultMemoize, {
        equalityCheck: (a: Branch[] | Repository | undefined | boolean, b: Branch[] | Repository | undefined | boolean) => {
...
const branchSelectors = {
    ...selectors, selectEntities, selectById, selectByIds, selectByRef, selectByRoot, selectByGitdir, selectByRepo
};
