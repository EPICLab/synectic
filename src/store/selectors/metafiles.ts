...
const deepMetafilesEqualityCheck = (a: Metafile[], b: Metafile[]) =>
  filteredArrayEquality(a, b, ['id', 'modified']);
...
const selectVirtual = createSelector(selectAll, metafiles => metafiles.filter(isVirtualMetafile), {
...
const selectFiles = createSelector(selectAll, metafiles => metafiles.filter(isFileMetafile), {
  memoizeOptions: { resultEqualityCheck: deepMetafilesEqualityCheck }
});
...
const selectByFilepath = createCachedSelector(
  ...
  (metafiles, filepath, handlers) =>
    metafiles.filter(
      m => isEqualPaths(m.path, filepath) && (!isDefined(handlers) || handlers.includes(m.handler))
    )
)({
...
const metafileSelectors = {
  ...
  selectByState,
  selectByCards,
  selectConflictedByRepo,
  selectStagedByRepo,
  selectUnstagedByRepo
};
