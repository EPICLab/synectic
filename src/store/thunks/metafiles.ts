...
export const updateDirectoryMetafile = createAppAsyncThunk<DirectoryMetafile, FilebasedMetafile>(
  ...
  async (metafile, thunkAPI) => {
    ...
    const contains = (
      await descendants.reduce(async (accumulator: Promise<Metafile[]>, current) => {
        ...
        else metafiles.push(await thunkAPI.dispatch(createMetafile({ path: current })).unwrap());
        return metafiles;
      }, Promise.resolve([]))
    ).map(metafile => metafile.id);
...
export const updateFilebasedMetafile = createAppAsyncThunk<
  DirectoryMetafile | FileMetafile,
  FilebasedMetafile
...
export const updateVersionedMetafile = createAppAsyncThunk<
  VersionedMetafile | FilebasedMetafile,
  FilebasedMetafile
>('metafiles/updateVersionedMetafile', async (metafile, thunkAPI) => {
  ...
  if (isDirectoryMetafile(metafile)) {
    ...
    const statuses = root
      ? (await worktreeStatus({ dir: root, pathspec: metafile.path }))?.entries
      : undefined;

    if (isDefined(statuses)) {
      const [newEntries, updatedMetafiles, unmodifiedMetafiles] = symmetrical(
        statuses,
        descendants,
      ...
      updatedMetafiles.map(([status, metafile]) => {
      ...
      unmodifiedMetafiles.map(metafile => {
        if (metafile.status !== 'unmodified')
          thunkAPI.dispatch(metafileUpdated({ ...metafile, status: 'unmodified' }));
      });
  ...
  const conflicted: Awaited<ReturnType<typeof checkUnmergedPath>> = await checkUnmergedPath(
    metafile.path
  );
 ...
 */
export const switchBranch = createAppAsyncThunk<
  Metafile | undefined,
  { metafileId: UUID; cardId: UUID; ref: string; root: PathLike }
>('metafiles/switchBranch', async ({ metafileId, ref, root }, thunkAPI) => {
  const state = thunkAPI.getState();

  const metafile = metafileSelectors.selectById(state, metafileId);
  if (!metafile || !isFilebasedMetafile(metafile) || !isVersionedMetafile(metafile))
    return undefined;
