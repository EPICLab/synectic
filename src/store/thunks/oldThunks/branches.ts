...
import { getBranchRoot, getRoot, getWorktreePaths } from '../../../containers/git';
import { checkout, currentBranch, getConfig, log } from '../../../containers/old-git/git-porcelain';
...
import * as branches from '../branches'; // eslint-disable-line @typescript-eslint/no-unused-vars

type BranchIdentifiers = { root: PathLike, branch: string, scope: 'local' | 'remote' };

/**
 * @deprecated This implementation uses old-git functions that rely on isomorphic-git, please use {@link branches.fetchBranch} instead.
 */
...
export const createBranch = createAsyncThunk<Branch, BranchIdentifiers, AppThunkAPI>(
    ...
    async (identifiers, thunkAPI) => {
        ...
        return thunkAPI.dispatch(branchAdded({
            ...
            status: 'clean',
            // commits: commits,
            commits: [], // WARNING: Temporary fix to allow this file to not throw errors, but the commented out line above is the original behavior
            head: head
        })).payload;
    }
);
...
/**
 * @deprecated This implementation uses old-git functions that rely on isomorphic-git, please use {@link branches.fetchBranches} instead.
 */
 ...
 * option is used).
 * 
 * @deprecated This implementation uses old-git functions that rely on isomorphic-git, please use {@link branches.switchBranch} instead.
 */
