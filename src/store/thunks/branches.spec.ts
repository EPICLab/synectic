...
describe('thunks/branches', () => {
  ...
  //       .dispatch(fetchBranch({ branchIdentifiers: { root: 'foo/', ref: 'main', scope: 'local' } }))
  //       .unwrap();
  //     expect(branch).toStrictEqual(mockedBranch1);
  //   });

  //   it('fetchBranches resolves existing branches via root path', async () => {
  //     jest.spyOn(gitShowBranch, 'showBranch').mockResolvedValue([
  //       { ref: 'test', remote: 'origin', scope: 'local' },
  ...
  //     expect.assertions(2);
  //     const branch = await store
  //       .dispatch(buildBranch({ root: 'foo/', ref: 'main', scope: 'local' }))
  //       .unwrap();
  //     expect(isUUID(branch.id.toString())).toBe(true);
