...
const BaseStack = forwardRef<HTMLDivElement, Props>(({ id, ...props }, ref) => {
  ...
  return (
    <StackComponent {...props} x={stack?.x} y={stack?.y} cards={stack?.cards} ref={mergedRefs}>
      {!props.dragOverlay ? (
      ...
      {isDiffable() ? (
        <Tooltip title={'Generate the diff of these cards'} placement="top-end">
          <IconButton
      ...
      <ErrorBoundary fallback={<Error>💥Child Error💥</Error>}>
        <DndStackContext id={id} ref={localRef}>
          <SortableContext items={stack?.cards ?? []} strategy={verticalListSortingStrategy}>
            {stack?.cards.map((id, index) => <Card key={id} id={id} sortableIndex={index} />)}
          </SortableContext>
        </DndStackContext>
      </ErrorBoundary>
    </StackComponent>
