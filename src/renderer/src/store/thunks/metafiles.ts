...
import { CardType, FilesystemStatus, PathLike, UUID } from 'types/app';
import { ExactlyOne } from 'types/util';

const extractFilename = window.api.fs.extractFilename;
const hasUpdates = window.api.utils.hasUpdates;
 ...
 */
export const fetchMetafile = createAppAsyncThunk<
  Metafile,
  { path: PathLike; handlers?: CardType[] }
>('metafiles/fetchMetafile', async ({ path: filepath, handlers }, thunkAPI) => {
...
>('metafiles/fetchParent', async (metafile, thunkAPI) => {
  const metafiles: ReturnType<typeof metafileSelectors.selectByFilepath> =
    metafileSelectors.selectByFilepath(
      thunkAPI.getState(),
      window.api.fs.dirname(metafile.path.toString())
    );
...
>('metafiles/updateDirectoryMetafile', async ({ id, shallow = false }, thunkAPI) => {
  ...
  const contains: UniqueIdentifier[] = [];
  if (!shallow) {
    const descendants = await window.api.fs.getDescendantPaths(metafile.path);
    await Promise.all(
...
export const updateFileMetafile = createAppAsyncThunk<FileMetafile | undefined, UUID>(
  ...
  async (id, thunkAPI) => {
    ...
    const content = (
      await window.api.fs.readFileAsync(metafile.path, { encoding: 'utf-8' })
    ).toString();
...
>('metafiles/updateVersionedMetafile', async (id, thunkAPI) => {
  ...
  if (isDirectoryMetafile(metafile)) {
    const root = await window.api.git.getRoot(metafile.path);
    const descendants = root ? metafileSelectors.selectByRoot(thunkAPI.getState(), root) : [];
    const statuses = root
      ? (await window.api.git.worktreeStatus({ dir: root, pathspec: metafile.path }))?.entries
      : undefined;
    ...
    if (isDefined(statuses)) {
      const [newEntries, updatedMetafiles, unmatchedMetafiles] = symmetrical(
        statuses,
        descendants,
        (status, descendant) => window.api.fs.isEqualPaths(status.path, descendant.path)
      );
        ...
        unmatchedMetafiles.map(async metafile => {
          const matcher = (directory: string): boolean => {
            const match = statuses.find(status =>
              window.api.fs.isEqualPaths(status.path, directory)
            );
            return isDefined(match);
          };

          const match = await window.api.git.getMatchUp(metafile.path, matcher);
          const parentStatus = match
            ? statuses.find(status => window.api.fs.isEqualPaths(status.path, match))
            : undefined;
  ...
  const status = await window.api.git.fileStatus(metafile.path);
  const conflicted = await window.api.git.checkUnmergedPath(metafile.path);
...
export const saveFile = createAppAsyncThunk<boolean, { id: UUID; filepath?: string }>(
  ...
  async ({ id, filepath }, thunkAPI) => {
    ...
    if (
      ...
      // write file
      await window.api.fs.writeFileAsync(targetPath, metafile.content);
...
export const switchBranch = createAppAsyncThunk<Metafile | undefined, { id: UUID; ref: string }>(
  ...
  async ({ id, ref }, thunkAPI) => {
    ...
    const { dir } = await window.api.git.getWorktreePaths(metafile?.path ?? '');
    ...
    if (isVersionedMetafile(metafile) && branchA && branchB) {
      ...
      const relativePath = window.api.fs.relative(
        branchA.root.toString(),
        metafile.path.toString()
      );
      // absolute path from new linked worktree root to file
      const absolutePath = window.api.fs.join(branchB.root.toString(), relativePath);
...
export const revertChanges = createAppAsyncThunk<void, VersionedMetafile>(
  ...
  async (metafile, thunkAPI) => {
    ...
    if (metafile.status !== 'unmodified' && branch) {
      await window.api.git.checkoutPathspec({
        dir: branch.root,
