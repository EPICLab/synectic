...
import type { MergeAction, MergeOutput, SHA1, UUID } from 'types/app';
import type { Branch, UnmergedBranch } from 'types/branch';
...
const partition = window.api.utils.partition;
const removeObjectProperty = window.api.utils.removeObjectProperty;
...
export const buildBranch = createAppAsyncThunk<Branch | undefined, BranchIdentifiers>(
  ...
  async (identifiers, thunkAPI) => {
    ...
    const { dir, gitdir, worktreeDir, worktreeGitdir } =
      await window.api.git.getWorktreePaths(root);
