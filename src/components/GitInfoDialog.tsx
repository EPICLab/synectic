...
export const GitInfoDialog: React.FunctionComponent<GitInfoDialogProps> = props => {
  ...
  const validateUsername = (name: string): boolean => /^\w+(-\w+)?$/.test(name);
  // Email addresses must adhere to RFC 5322 (https://www.ietf.org/rfc/rfc5322.txt), but this approximate validation is good enough
  ...
  const checkGitConfig = async (isChecked: boolean, keyPath: string): Promise<string> => {
    ...
    if (!isChecked) return result.scope === 'local' ? result.value : '';
    else return result.scope !== 'none' ? result.value : '';
  };
    ...
    <Dialog id='git-info-dialog' open={props.open} onClose={handleClose} aria-labelledby='git-info-dialog' >
      <div className='git-info-dialog-container'>
        ...
        <TextField
          ...
          value={name}
          error={name.length > 0 && !validateUsername(name)}
