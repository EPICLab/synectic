...
const SaveButton = ({
  ...
  const modified = metafiles.filter(
    m =>
      (isFileMetafile(m) &&
        createHash('md5').update(m.content).digest('hex') !== cache[m.path.toString()]?.content) ||
  ...
  const save = async (event: React.MouseEvent) => {
    ...
    await Promise.all(
      modified.filter(isFileMetafile).map(async metafile => {
  ...
  const onHover = () => {
    if (cards.length > 1) {
      cards
        .filter(c => (modified.find(m => c.metafile === m.id) ? true : false))
        .map(c =>
