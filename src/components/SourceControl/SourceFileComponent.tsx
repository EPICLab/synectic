...
const SourceFileComponent = (props: { metafileId: UUID }) => {
  const metafile = useAppSelector((root: RootState) =>
    metafileSelectors.selectById(root, props.metafileId)
  );
  ...
  const handleLabelClick = async () => {
    if (metafile && isVersionedMetafile(metafile)) {
      isStaged(metafile.status)
        ? await unstage()
        : isModified(metafile.status)
        ? await stage()
        : undefined;
    }
  };
